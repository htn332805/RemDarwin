...
from datetime import datetime, timezone  # Import datetime class to handle date and time operations, particularly for parsing expiration dates and calculating days to expiration; this is crucial for options data processing in RemDarwin, as expiration timing affects option pricing and strategy decisions.
...
class YFinanceOptionChainFetcher:  # Define the YFinanceOptionChainFetcher class as the main component for fetching and processing options data from Yahoo Finance; this class encapsulates the logic for retrieving option chains, parsing data, and validating contracts, serving as the core engine for options data acquisition in RemDarwin's systematic trading system.
    ...
    def check_last_fetch_timestamp(self, symbol: str, max_age_hours: int = 24) -> bool:
        """
        Check if options data for the symbol was fetched recently.
        ...
        Returns:
            bool: True if data is recent and doesn't need update, False if update needed
        """
        db_name = f"{symbol}_options.db"
        if not os.path.exists(db_name):
            return False  # No database exists, need to fetch

        try:
            conn = sqlite3.connect(db_name)
            cursor = conn.cursor()

            # Check if options_history table exists and has recent data
            ...
            cursor.execute("SELECT MAX(fetch_timestamp) FROM options_history")
            ...
            try:
                last_fetch = datetime.fromisoformat(result[0].replace('Z', '+00:00'))
            ...
            self.logger.warning(f"Error checking last fetch timestamp for {symbol}: {e}")
...
if __name__ == "__main__":  # Execute demo code only when script is run directly; this prevents demo execution when the module is imported by RemDarwin's main application.
    ...
    parser.add_argument('--force-update', action='store_true', help='Force update options data even if recently fetched')  # Add force update flag; this allows forcing a fresh data fetch regardless of cache status in RemDarwin.
    ...
    print(f"Parsed arguments: ticker={args.ticker}, force_update={args.force_update}")  # Print parsed arguments for verification; this provides user feedback on the parsed CLI inputs in RemDarwin.
    ...
    if not args.force_update and fetcher.check_last_fetch_timestamp(ticker_symbol):
        print(f"Data for {ticker_symbol} is recent (last updated within 24 hours). Use --force-update to refresh.")
        sys.exit(0)
    ...
    if result['puts']:  # Check if puts data is available for display; this ensures complete demo coverage in RemDarwin's testing.
        ...
        print(sample_put)  # Display the sample put contract details; this completes the demonstration of RemDarwin's options data structure and fetching capabilities.</content>
