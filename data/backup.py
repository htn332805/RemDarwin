...
class CompFin:
    ...
    def compute_earning_yield_metric(self, earnings_per_share, price_per_share):
        ...
        """
        self.earnings_per_share = earnings_per_share
        self.market_price_per_share = price_per_share
        if self.earnings_per_share is None or self.market_price_per_share is None:
            raise ValueError("Missing required input: earnings_per_share or price_per_share")
        if not isinstance(self.earnings_per_share, (int, float)) or not isinstance(self.market_price_per_share, (int, float)):
            raise ValueError("Inputs must be numeric")
        if self.market_price_per_share == 0:
            raise ValueError("Division by zero: price_per_share cannot be zero")
        return self.earnings_per_share / self.market_price_per_share
...
def main():
    ...
    if path_exists(input_file_name):
        ...
        #ic(list(df.columns))    
        #[print(col) for col in df.columns if "cashandcashequivalentsatcarryingvalue" in col.lower()]
        #print(df.tail(1).to_dict(orient="records")[0]["cashandcashequivalentsatcarryingvalue"])
        data.accounts_receivable = df.tail(1).to_dict(orient="records")[0]["accountsreceivablenetcurrent"]  
        data.total_revenue = df.tail(1).to_dict(orient="records")[0]["revenuefromcontractwithcustomerexcludingassessedtax"]  
        ...
        data.total_current_liabilities = df.tail(1).to_dict(orient="records")[0]["liabilitiescurrent"]
        data.total_current_assets = df.tail(1).to_dict(orient="records")[0]["assetscurrent"]
        data.current_assets = df.tail(1).to_dict(orient="records")[0]["assetscurrent"]
        data.goodwill_and_intangible_assets = df.tail(1).to_dict(orient="records")[0]["goodwill"]+df.tail(1).to_dict(orient="records")[0]["intangibleassetsnetexcludinggoodwill"]
        data.total_liabilities = df.tail(1).to_dict(orient="records")[0]["liabilities"]
        ...
        cashandcash1=df.tail(1).to_dict(orient="records")[0]["cashandcashequivalentsatcarryingvalue"]
        cashandcash2=df.tail(1).to_dict(orient="records")[0]["restrictedcashandcashequivalentsatcarryingvalue"]
        cashandcash3=df.tail(1).to_dict(orient="records")[0]["othercurrentassetsfairvaluedisclosure"]
        data.cash_and_cash_equivalents = cashandcash1+cashandcash2+cashandcash3
        data.depreciation_and_amortization = df.tail(1).to_dict(orient="records")[0]["depreciationamortizationandother"]
        data.dividends_per_share = df.tail(1).to_dict(orient="records")[0]["commonstockdividendspersharedeclared"]
        data.general_and_administrative_expenses = df.tail(1).to_dict(orient="records")[0]["generalandadministrativeexpense"]
        data.gross_profit = df.tail(1).to_dict(orient="records")[0]["grossprofit"]
        data.market_price_per_share=65.0
        data.growth_rate=0.1
        data.market_cap=data.number_of_shares_outstanding*data.market_price_per_share
        _val=df.tail(1).to_dict(orient="records")[0]["inventorynet"]
        data.operatingleaseliabilitynoncurrent = _val if _val else 0
        data.long_term_debt = df.tail(1).to_dict(orient="records")[0]["longtermdebtnoncurrent"]+ data.operatingleaseliabilitynoncurrent
        short_term_debt = df.tail(1).to_dict(orient="records")[0]["debtcurrent"]
        data.total_debt = short_term_debt + data.long_term_debt
        data.ebitda=data.operating_income+data.depreciation_and_amortization
        data.enterprise_value= data.total_debt - cashandcash1+ data.market_cap
        data.income_before_tax = df.tail(1).to_dict(orient="records")[0]["incomelossfromcontinuingoperationsbeforeincometaxesextraordinaryitemsnoncontrollinginterest"]
        data.total_assets = df.tail(1).to_dict(orient="records")[0]["assets"]
        data.earnings_per_share = df.tail(1).to_dict(orient="records")[0]["earningspersharebasic"]
        data.price_earnings_ratio = data.market_price_per_share / data.earnings_per_share if data.earnings_per_share else None
        data.inventory = df.tail(1).to_dict(orient="records")[0]["inventorynet"]
        data.inventory = df.tail(1).to_dict(orient="records")[0]["inventorynet"]
        data.inventory = df.tail(1).to_dict(orient="records")[0]["inventorynet"]
        data.inventory = df.tail(1).to_dict(orient="records")[0]["inventorynet"]
